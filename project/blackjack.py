...
#python3 -i blackjack.py
#python3 -m doctest blackjack.py
...
class Blackjack:
    ...
    def __init__(self, wallet):
        ...
        assert isinstance(self.round_number, int)
    ...
    def play_round(self, num_rounds, stand_threshold):
        ...
        for i in range(num_rounds):
            ...
            if self.wallet < bet:
                ...
                ' is less than bet amount $' + str(bet) + '.'
                bet = 5
                break
            else:
                ...
                mon = randint(0, 5)
                mod = randint(0, 5)
                self.deck.shuffle(modified_overhand=mod, mongean=mon)
                for i in range(2):
                ...
                winner = self.determine_winner(Blackjack.calculate_score(player), \
    ...
    def calculate_score(hand):
        ...
        jqk = ['J', 'Q', 'K']
        to_int = lambda x: 10 if x.rank in jqk else 1 \
        if x.rank =='A' else x.rank
        rank_as_int = list(map(to_int, hand.cards))
        val = sum([i for i in rank_as_int])
        if val <= 11 and any(card.rank == 'A' for card in hand.cards):
            val +=10
        return val 
    ...
    def determine_winner(self, player_score, dealer_score):
        ...
        assert isinstance(dealer_score,int)


        if ((player_score > dealer_score) and player_score <=21) or \
    ...
    def hit_or_stand(self, hand, stand_threshold):
        ...
        while Blackjack.calculate_score(hand) < stand_threshold and (len(self.deck.get_cards()) > 0):
            if isinstance(hand, DealerHand):
                self.log += 'Dealer pulled a ' + repr(self.deck.cards[0]) + '\n'
            else:
                self.log += 'Player pulled a ' + repr(self.deck.cards[0]) + '\n'
    ...
    def add_to_file(self, player_hand, dealer_hand, result):
        ...
        str_base = 'ROUND ' + str(self.round_number) + ':\n'+\
        'Player Hand:\n' + str(player_hand) + '\nDealer Hand:\n' + \
        str(dealer_hand) + '\nWinner of ROUND ' + str(self.round_number) +\
        ': ' + result + '\n' 

        with open('game_summaries/game_summary' + \
        str(self.game_number) + '.txt', 'a', encoding = "utf-8") as f:
            f.write(str_base + '\n')
